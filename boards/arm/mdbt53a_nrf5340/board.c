...
LOG_MODULE_REGISTER(mdbt53a_board_init);

/* Initialization chain of MDBT53A board requires some delays before on board sensors
...
BUILD_ASSERT(CONFIG_MDBT53A_INIT_PRIORITY > CONFIG_REGULATOR_FIXED_INIT_PRIORITY,
	"CONFIG_MDBT53A_INIT_PRIORITY must be higher than CONFIG_REGULATOR_FIXED_INIT_PRIORITY");
#endif /* CONFIG_REGULATOR_FIXED */
#if defined(CONFIG_IEEE802154_NRF5)
BUILD_ASSERT(CONFIG_MDBT53A_INIT_PRIORITY < CONFIG_IEEE802154_NRF5_INIT_PRIO,
	"CONFIG_MDBT53A_INIT_PRIORITY must be less than CONFIG_IEEE802154_NRF5_INIT_PRIO");
#endif /* CONFIG_IEEE802154_NRF5 */
#endif /* !CONFIG_TRUSTED_EXECUTION_SECURE */

#if defined(CONFIG_SENSOR)
BUILD_ASSERT(CONFIG_MDBT53A_INIT_PRIORITY < CONFIG_SENSOR_INIT_PRIORITY,
	"CONFIG_MDBT53A_INIT_PRIORITY must be less than CONFIG_SENSOR_INIT_PRIORITY");
#endif
...
static int setup(void)
{
	...
	if (IS_ENABLED(CONFIG_SENSOR)) {
		/* Initialization chain of MDBT53A board requires some delays before on board
...
SYS_INIT(setup, POST_KERNEL, CONFIG_MDBT53A_INIT_PRIORITY);
